...
class InstagramRequest
{
    ...
    public function cache($name, $desc = false, $json = false)
    {
        ...
        if (file_exists($cache_file) and time() <= strtotime(
            '+' . $this->cache_time . ' minute',
    ...
    //KELİME BAŞLIYORSA
    public function start_with($samanlik, $igne)
    {
        $length = strlen($igne);
    ...
    //KELİME BİTİYORSA
    public function end_with($samanlik, $igne)
    {
        $length = strlen($igne);
        if ($length == 0) {
            return true;
